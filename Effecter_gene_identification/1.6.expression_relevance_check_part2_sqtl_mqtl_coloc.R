...
## load the SNP-metabo-gene pairs with significant sQTL associations
clsa_metabolite_cojo_SNPs_filtered_SNPID_gene5_sqtl_ALL1<-read.csv("./results/clsa_metabolite_cojo_SNPs_filtered_SNPID_gene5_sqtl_ALL1.csv",row.names = 1)
...
## the 3 information needed to load the sqtl summary stats files
#tissue, gene_id, CHR
coloc_result<-data.frame(
  ...
  metabo=character(),
  gene_id=character(),
  phenotype_id_sig=character(),
  phenotype_id_found=character(),
  tissue=character(),
  nsnps=integer(),
  PP.H0.abf=numeric(),
  PP.H1.abf=numeric(),
...
i=args[1]
set<-clsa_metabolite_cojo_SNPs_filtered_SNPID_gene5_sqtl_ALL1[i,]#7403 lines
testID_fixed_test<-set$testID_fixed
mqtl<-set$SNP.x
metabo<-set$metabolite
phenotype_id_sig<-set$phenotype_id#the phenotype_id found from GTEx significant sqtl file
...
file_name<-paste0(set$tissue,".v8.EUR.sqtl_allpairs.chr",set$CHR,".parquet")
df<- read_parquet(paste0("./data/GTEx_Analysis_v8_QTLs/GTEx_Analysis_v8_EUR_sQTL_all_associations/",file_name), as_tibble = TRUE)
...
d2=summary_stats_merged_filtered[,c(6,15:23)]
n_metabo<-max(d1$N)
n_sqtl<-max(d2$ma_count)
coloc.res <- coloc.abf(dataset1=list(pvalues=d1$P, 
                       ...
                       dataset2=list(beta=d2$slope, 
                                     ...
                                     N=n_sqtl,
                                     type="quant"))
temp_result<-data.frame(testID_fixed_test, mqtl,metabo,phenotype_id_sig,
                        gene_symbol,tissue,t(coloc.res$summary))

write.csv(summary_stats_merged_filtered,paste0("./restuls/coloc_results/sqtl/summary_stats/",i,"_summary_stats_",testID_fixed_test,"_",gene_symbol,"_",tissue,"_",".csv"))
write.csv(temp_result,paste0("./restuls/coloc_results/sqtl/",i,"_coloc_result_set",testID_fixed_test,"_",gene_symbol,"_",tissue,"_",".csv"))

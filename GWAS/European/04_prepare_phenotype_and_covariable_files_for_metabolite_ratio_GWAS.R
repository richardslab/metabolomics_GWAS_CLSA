...
## baseline data
Baseline_data_v3<-read.csv("./data/clsa_phenotypes/clsa_baseline_data", header=TRUE,stringsAsFactors=FALSE)
...
### list of individuals without metabolomics measurement or didnot' pass previous filter
...
for (n in 1:nrow(distinct_metabo_pairs)){
  ...
  metabo2<-distinct_metabo_pairs$metabolite_ID.y[n]
  dat_sub<-dat5 %>% select(ADM_GWAS_COM, metabo1, metabo2) %>% filter(is.na(get(metabo1)) != T &is.na(get(metabo2)) != T) %>% mutate(metabo_ratio=get(metabo1)/get(metabo2))
